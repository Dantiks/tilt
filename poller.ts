...
// Never hardcode the bot token: this file is part of a public repository, and a
// leaked token lets anyone read everything users send the bot.
const TOKEN = process.env.TELEGRAM_BOT_TOKEN ?? "";
const WEBHOOK_URL = process.env.TILTAP_WEBHOOK_URL ?? "http://localhost:3000/webhook/telegram";
const OFFSET_FILE = "poller_offset.txt";

if (!TOKEN) {
  console.error("[Poller] TELEGRAM_BOT_TOKEN is not set. Export it or put it in .env before starting the poller.");
  process.exit(1);
}
