...
См. заметку в Obsidian: `[[Projects/Tiltap]]` (в личном хранилище автора).
...
| WhatsApp webhook | ✅ | `GET`/`POST /webhook/whatsapp` — full bot on the Meta Cloud API |
| Public website | ✅ | Landing at `/`, web app at `/app` (`public/site`) |
...
```
Telegram Update ─┐
WhatsApp Message ┼→ routes/webhook.ts → controllers/{telegram,whatsapp}Controller.ts
Web / site ──────┘   routes/web.ts    → controllers/webController.ts
                                                ↓
                                    services/fileDownloadService.ts   (Telegram files)
                                    services/whatsappService.ts       (Cloud API + media)
...
All three channels share the transcription, cleanup and translation services;
...
The WhatsApp bot (`src/controllers/whatsappController.ts` + `src/services/whatsappService.ts`) mirrors the Telegram flow on the **Meta WhatsApp Cloud API**. It is enabled only when `WHATSAPP_ACCESS_TOKEN` and `WHATSAPP_PHONE_NUMBER_ID` are set; otherwise the webhook answers `200` and does nothing.

1. `GET /webhook/whatsapp` is Meta's subscription handshake (echoes `hub.challenge` when `hub.verify_token` matches). `POST` carries messages and delivery statuses; when `WHATSAPP_APP_SECRET` is set the `X-Hub-Signature-256` header is checked against the **raw** body, which `app.ts` stashes via the `express.json({ verify })` hook. The response is sent before any processing because Meta retries slow webhooks.
2. Message ids are deduplicated in memory for 10 minutes — Meta re-delivers until it gets a 200.
3. On first contact the interface language is guessed from the phone's country code (`996 → ky`, `992 → tg`, `998 → uz`, `7 → ru`, `1/44 → en`); WhatsApp sends no locale. Profiles live in `whatsapp_users`, pending confirmations in `whatsapp_pending_actions`.
4. Media or a supported link → source-language **list** → confirmation card (**Start / Languages / Cancel** buttons) → processing → result → 👍/👎/🛠 buttons.
5. Free-text commands work without a slash and in all five languages: `menu`, `help`, `settings`, `stop`, `translate`.

**Constraints that shape the UX** (do not "fix" these — they are platform limits):

- At most **3 reply buttons** per message and **10 list rows** per section; button titles are capped at 20 characters, list row titles at 24. `whatsappService.ts` truncates rather than letting the API reject the message.
- **Messages cannot be edited.** The Telegram spinner/progress bar has no equivalent, so `createPhaseReporter` sends at most 4 phase messages (download → transcribe → translate), with the Stop button on the first.
- Text bodies are capped at 4096 characters; `sendText` splits on paragraph boundaries and long results go out as a `.txt` document instead.
- The interface dictionary is **shared with Telegram** (`t()` in `telegramService.ts`), so wording never drifts between the bots. Those strings are Telegram HTML, so `htmlToWhatsApp()` converts `<b>` → `*`, `<i>` → `_`, `<code>` → ``` and unescapes entities. Strings that exist only on WhatsApp (rate prompt, Stop/Cancel labels) live in `WA_STRINGS` in `whatsappService.ts`.
- The 24-hour customer-service window applies: the bot can only send free-form replies within 24 hours of the user's last message.

WhatsApp requests are audited in `transcription_requests` via a new `whatsapp_wa_id` column, and feedback in `feedback` (`source = 'whatsapp'`, `whatsapp_wa_id`). The legacy `transcriptions`/`translations` tables are Telegram-only — their `telegram_chat_id` is `NOT NULL` — so the WhatsApp flow deliberately does not write to them; `transcription_requests` already stores the full text and segments.

## Public Website

`public/site` holds the customer-facing site, served by the same Express process and built the same way as the existing pages (static HTML + Tailwind/daisyUI from CDN, no build step):

- `/` — landing page: hero, features, language table, how it works, bot links, FAQ.
- `/app` — the full web app: upload / media link / text translation, SSE progress, timecoded segments, TXT/SRT/VTT export, re-translation into more languages, feedback.
- `/site/assets/i18n.js` — one dictionary for both pages in five languages; `/site/assets/app.js` — app logic.
- `GET /api/site/config` (`src/controllers/siteController.ts`) reports which channels are configured so the site never advertises a bot that does not exist. Set `TELEGRAM_BOT_USERNAME` / `WHATSAPP_CONTACT_PHONE` to show those buttons.
- Both pages follow the system light/dark preference with a manual toggle.

The original `/web` tool and `/web/admin.html` are untouched and still work; the new app reuses the same `/api/web/*` endpoints.
...
- `whatsapp_users` — the same three preferences keyed by `wa_id` (a phone number string, which cannot share `users.telegram_chat_id`)
- `whatsapp_pending_actions` — media buffers and links awaiting a language choice, so a confirmation survives a restart
...
| `WHATSAPP_ACCESS_TOKEN` | no | Meta permanent access token. Empty (or no phone number id) disables the WhatsApp bot entirely |
| `WHATSAPP_PHONE_NUMBER_ID` | no | Cloud API sender phone number **id**, not the number |
| `WHATSAPP_VERIFY_TOKEN` | no | Any string; echoed back during the `GET /webhook/whatsapp` handshake |
| `WHATSAPP_APP_SECRET` | no | Verifies `X-Hub-Signature-256`. Without it the webhook accepts anything that reaches the URL |
| `WHATSAPP_API_VERSION` | no | Graph API version (default `v21.0`) |
| `WHATSAPP_GRAPH_URL` | no | Graph host (default `https://graph.facebook.com`); override only for local testing |
| `TELEGRAM_BOT_USERNAME` | no | Shows the Telegram button on the public site |
| `WHATSAPP_CONTACT_PHONE` | no | Shows the WhatsApp button on the public site (digits only) |
