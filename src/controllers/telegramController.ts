...
function buildSettingsMenuText(prefs: UserPreferences): string {
  ...
  const targetLabel = prefs.targetLanguage === "none" ? t("noDefaultTarget", lang) : `${LANGUAGE_LABELS[prefs.targetLanguage]}`;

  return `${t("settingsMenu", lang)}\n\n` +
    `${t("settingsInterfaceLanguage", lang)}: ${LANGUAGE_LABELS[lang]}\n` +
