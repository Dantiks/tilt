...
-- WhatsApp users. Kept in their own table rather than folded into `users`:
-- a wa_id is a phone number string, not a Telegram chat id, and the unique
-- constraint on users.telegram_chat_id has no room for a second identity space.
CREATE TABLE IF NOT EXISTS whatsapp_users (
  id SERIAL PRIMARY KEY,
  wa_id VARCHAR(32) UNIQUE NOT NULL,     -- WhatsApp phone number id (digits, no +)
  profile_name VARCHAR(255),
  preferred_language VARCHAR(10),        -- default transcription language
  interface_language VARCHAR(10) DEFAULT 'ru',
  target_language VARCHAR(10),           -- default translation language, or 'none'
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Mirrors pending_actions so a confirmation survives a restart.
CREATE TABLE IF NOT EXISTS whatsapp_pending_actions (
  id SERIAL PRIMARY KEY,
  wa_id VARCHAR(32) UNIQUE NOT NULL,
  action_id VARCHAR(64) NOT NULL,
  action_type VARCHAR(20) NOT NULL,      -- 'media' | 'youtube' | 'translate_text'
  payload JSONB NOT NULL,
  buffer BYTEA,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_whatsapp_pending_actions_wa_id ON whatsapp_pending_actions(wa_id);

-- WhatsApp requests and feedback reuse the shared audit tables; they only need
-- their own identity column, since wa_id does not fit telegram_chat_id.
ALTER TABLE feedback ADD COLUMN IF NOT EXISTS whatsapp_wa_id VARCHAR(32);
ALTER TABLE transcription_requests ADD COLUMN IF NOT EXISTS whatsapp_wa_id VARCHAR(32);

CREATE INDEX IF NOT EXISTS idx_feedback_wa_id ON feedback(whatsapp_wa_id);
CREATE INDEX IF NOT EXISTS idx_transcription_requests_wa_id ON transcription_requests(whatsapp_wa_id);
